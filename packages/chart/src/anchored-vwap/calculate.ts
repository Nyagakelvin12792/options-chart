...
export function calculateAnchoredVwap(input: AnchoredVwapInput): AnchoredVwapResult {
  ...
  for (let i = anchorIndex; i < sortedCandles.length; i++) {
    ...
    if (volume === 0) {
      // If zero volume, keep previous VWAP and std dev, but emit point
      if (cumVolume === 0) {
        // Initial candle with 0 volume
        runningVwap = price;
        runningM2 = 0;
        runningVariance = 0;
        runningStdDev = 0;
