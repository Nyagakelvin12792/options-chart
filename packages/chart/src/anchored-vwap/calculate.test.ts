...
describe("Anchored VWAP Pure Calculation", () => {
  ...
  it("honors replayCutoff and excludes later or unclosed candles", () => {
    ...
    expect(result.exclusions.some((e) => e.reason === "after_replay_cutoff")).toBe(true);
  });

  it("carries forward VWAP when candle volume is zero without NaN", () => {
    const c1 = makeCandle({ openTime: 1_000, closeTime: 1_059, close: 100, volume: 10 });
    const c2 = makeCandle({ openTime: 1_060, closeTime: 1_119, close: 150, volume: 0 });
  ...
  it("automatically sorts out-of-order candles", () => {
    const c1 = makeCandle({ openTime: 1_000, closeTime: 1_059, close: 100, volume: 10 });
    const c2 = makeCandle({ openTime: 2_000, closeTime: 2_059, close: 120, volume: 10 });
    const result = calculateAnchoredVwap({
      anchorTimestamp: 1_000,
