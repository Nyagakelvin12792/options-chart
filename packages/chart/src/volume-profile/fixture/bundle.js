...
// packages/chart/src/volume-profile/volume-profile-primitive.ts
var VolumeProfilePaneRenderer = class {
  constructor(getCoords, getPocCoord, getVahCoord, getValCoord, getMaxVolume, getPresentation) {
    this.getCoords = getCoords;
    this.getPocCoord = getPocCoord;
    this.getVahCoord = getVahCoord;
    this.getValCoord = getValCoord;
    ...
    target.useBitmapCoordinateSpace(
      ({ context, horizontalPixelRatio, verticalPixelRatio, bitmapSize }) => {
        ...
        for (const item of coords) {
          ...
          const h2 = Math.max(1 * verticalPixelRatio, y2 - y1);
          if (y2 < 0 || y1 > bitmapSize.height) continue;
          const row = item.row;
          const barWidth = row.totalVolume / maxVol * maxWidthPx;
          if (barWidth <= 0) continue;
          const opacity = showValueAreaShading && !row.isValueArea ? nonValueAreaOpacity : valueAreaOpacity;
          context.globalAlpha = opacity;
          const hasDirectional = (row.bullishVolume > 0 || row.bearishVolume > 0 || row.neutralVolume > 0) && row.bullishVolume + row.bearishVolume + row.neutralVolume > 0;
          if (hasDirectional) {
            ...
            if (placement === "right") {
              ...
              if (neutW > 0) {
                ...
                context.fillRect(currentRight - neutW, y1, neutW, h2);
                currentRight -= neutW;
              }
              if (bearW > 0) {
                context.fillStyle = bearishColor;
                context.fillRect(currentRight - bearW, y1, bearW, h2);
                currentRight -= bearW;
              }
              if (bullW > 0) {
                context.fillStyle = bullishColor;
                context.fillRect(currentRight - bullW, y1, bullW, h2);
              }
            } else {
              let currentLeft = 0;
              if (bullW > 0) {
                context.fillStyle = bullishColor;
                context.fillRect(currentLeft, y1, bullW, h2);
                currentLeft += bullW;
              }
              if (bearW > 0) {
                context.fillStyle = bearishColor;
                context.fillRect(currentLeft, y1, bearW, h2);
                currentLeft += bearW;
              }
              if (neutW > 0) {
                context.fillStyle = neutralColor;
                context.fillRect(currentLeft, y1, neutW, h2);
              }
            }
          } else {
            context.fillStyle = totalColor;
            if (placement === "right") {
              const x2 = bitmapSize.width - barWidth;
              context.fillRect(x2, y1, barWidth, h2);
            } else {
              context.fillRect(0, y1, barWidth, h2);
            }
          }
        }
        context.globalAlpha = 1;
        if (showVAH && yVah !== null) {
          const y2 = Math.round(yVah * verticalPixelRatio);
          if (y2 >= 0 && y2 <= bitmapSize.height) {
        ...
        if (showVAL && yVal !== null) {
          const y2 = Math.round(yVal * verticalPixelRatio);
          if (y2 >= 0 && y2 <= bitmapSize.height) {
        ...
        if (showPOC && yPoc !== null) {
          const y2 = Math.round(yPoc * verticalPixelRatio);
          if (y2 >= 0 && y2 <= bitmapSize.height) {
...
var VolumeProfilePaneView = class {
  paneRenderer;
  constructor(getCoords, getPocCoord, getVahCoord, getValCoord, getMaxVolume, getPresentation) {
...
var VolumeProfilePriceAxisView = class {
...
var VolumeProfilePrimitive = class {
  ...
  updateAllViews() {
    ...
    this.yPoc = this.result.pocPrice !== null ? series.priceToCoordinate(this.result.pocPrice) : null;
    this.yVah = this.result.vah !== null ? series.priceToCoordinate(this.result.vah) : null;
    this.yVal = this.result.val !== null ? series.priceToCoordinate(this.result.val) : null;
  }
  paneViews() {
...
var LightweightChartsAdapter = class {
  ...
  volumeProfilePrimitives = /* @__PURE__ */ new Map();
  ...
  setVolumeProfile(id, renderInput) {
    const series = this.requireSeries();
...
// packages/chart/src/volume-profile/calculate.ts
var CALCULATION_VERSION = "1.0.0";
var MAX_ROW_COUNT = 2048;
function calculateVolumeProfile(input) {
  const {
    candles,
    range,
    replayCutoff,
  ...
  const seenTimestamps = /* @__PURE__ */ new Map();
  const validCandles = [];
  for (const candle of candles) {
    ...
    const rawVolume = volumeUnit === "quote" ? candle.quoteVolume : candle.volume;
    if (rawVolume === void 0 || rawVolume === null || Number.isNaN(rawVolume)) {
    ...
    if (existing) {
      if (existing.closeTime === candle.closeTime && existing.open === candle.open && existing.high === candle.high && existing.low === candle.low && existing.close === candle.close && existing.volume === candle.volume && existing.quoteVolume === candle.quoteVolume) {
  ...
  validCandles.sort((a2, b2) => a2.openTime - b2.openTime);
  ...
  for (const candle of validCandles) {
    ...
    if (candle.openTime < range.from || candle.closeTime > range.to) {
      const overlaps = candle.openTime < range.from && candle.closeTime > range.from || candle.openTime < range.to && candle.closeTime > range.to;
      if (overlaps) {
  ...
  for (let i = 1; i < eligibleCandles.length; i++) {
    ...
    if (prev && curr && curr.openTime > prev.closeTime + 1e3) {
  ...
  for (const candle of eligibleCandles) {
    ...
    if (candle.high === candle.low) {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        const inBin = isLast ? candle.low >= row.low && candle.low <= row.high : candle.low >= row.low && candle.low < row.high;
        if (inBin) {
          allocatedIdx = i;
          break;
        }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const overlap = Math.max(0, Math.min(candle.high, row.high) - Math.max(candle.low, row.low));
  ...
  while (accumulatedValueAreaVol < targetVolume && (idxAbove < rows.length || idxBelow >= 0)) {
    const rowAbove = idxAbove < rows.length ? rows[idxAbove] : void 0;
    const rowBelow = idxBelow >= 0 ? rows[idxBelow] : void 0;
...
// packages/chart/src/volume-profile/cache.ts
function buildVolumeProfileCacheKey(input) {
  const meta = input.sourceMetadata;
  const binMode = input.binConfig.mode;
  const binValue = input.binConfig.mode === "rowCount" ? input.binConfig.rowCount : input.binConfig.binSize;
  const tickOrigin = input.binConfig.mode === "binSize" ? input.binConfig.tickOrigin ?? 0 : 0;
...
// packages/chart/src/volume-profile/controller.ts
...
// packages/chart/src/volume-profile/fixture/fixture-data.ts
function getFixtureCandles() {
  const candles = [];
  const start = 17e11;
  const interval = 6e4;
  let price = 65e3;
  for (let i = 0; i < 200; i++) {
    ...
    candles.push({
      ...
      tradeCount: 25,
      isClosed: i < 180
      // Last 20 candles forming/unclosed for replay testing
    });
    price = close;
  }
  return candles;
...
// packages/chart/src/volume-profile/fixture/harness.ts
document.addEventListener("DOMContentLoaded", () => {
  ...
  let directionMode = "total";
  let binConfig = { mode: "rowCount", rowCount: 50 };
  let isReplay = false;
  ...
  function updateInput() {
    ...
    const input = {
      ...
      replayCutoff: isReplay ? replayCutoff : void 0,
      binConfig,
      directionMode,
      valueAreaPercent: 70,
      sourceMetadata: {
  ...
  btnDirection.addEventListener("click", () => {
    ...
    btnDirection.textContent = directionMode === "candle-direction" ? "Direction: Candle" : "Direction: Total";
    updateInput();
  });
  const selBins = document.getElementById("sel-bins");
