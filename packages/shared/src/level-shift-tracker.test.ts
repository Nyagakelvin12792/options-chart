...
describe("LevelShiftTracker (Shift-Aware Level Model)", () => {
  ...
  it("tracks identical confluence zone ids independently across expiry scopes", () => {
    const tracker = new LevelShiftTracker();
    const zone: ConfluenceZoneInput = {
      id: "zone-1",
      priceLow: 60_000,
      priceHigh: 61_000,
      score: 80,
    ...
    const allRecord = tracker.updateConfluenceZones([zone], "all", 1_000)[0]!;
    const nearRecord = tracker.updateConfluenceZones([zone], "near", 2_000)[0]!;

    expect(allRecord.id).toBe("confluence-zone-1-all");
    expect(nearRecord.id).toBe("confluence-zone-1-near");
    expect(nearRecord.activationTimestamp).toBe(2_000);
  });
