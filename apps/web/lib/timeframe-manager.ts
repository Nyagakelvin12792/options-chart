...
export class TimeframeManager {
  ...
  async switchTimeframe(options: TimeframeSwitchOptions): Promise<void> {
    ...
    // 1. Paint cached data immediately, then refresh and resume background fill.
    ...
    try {
      ...
      const initialCandles = Array.from(
        new Map(
          [...(cached ?? []), ...initial.candles].map((candle) => [
            candle.openTime,
            candle,
          ]),
        ).values(),
      ...
      if (!cached?.length) {
        ...
        onBackgroundProgress?.(initialCandles, {
          totalCandles: initialCandles.length,
          targetBars,
          interval,
        });
      }
