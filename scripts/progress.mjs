...
const journalDirectory = join(repositoryRoot, "docs", "progress", "M0");
const startMarker = "<!-- progress:M0:start -->";
const endMarker = "<!-- progress:M0:end -->";

const journalFiles = readdirSync(journalDirectory)
  .filter((fileName) => /^M0\.\d+\.md$/.test(fileName))
  .sort((left, right) => {
    const leftNumber = Number(left.match(/^M0\.(\d+)\.md$/)?.[1]);
    const rightNumber = Number(right.match(/^M0\.(\d+)\.md$/)?.[1]);
    return leftNumber - rightNumber;
  });

const tasks = journalFiles.map((fileName) => {
  const contents = readFileSync(join(journalDirectory, fileName), "utf8");
  const heading = contents.match(/^# (M0\.\d+) (.+)$/m);
...
const replaceGeneratedSection = (contents) => {
  const start = contents.indexOf(startMarker);
  const end = contents.indexOf(endMarker);

  if (start < 0 || end < start) {
    throw new Error("PROGRESS.md does not contain the M0 generation markers");
  }

  return `${contents.slice(0, start)}${generatedSection}${contents.slice(end + endMarker.length)}`;
};
...
if (mode === "build") {
  const updatedProgress = replaceGeneratedSection(currentProgress);
  if (updatedProgress !== currentProgress) {
    const { writeFileSync } = await import("node:fs");
    writeFileSync(progressPath, updatedProgress, "utf8");
  }
} else if (mode === "check") {
  if (replaceGeneratedSection(currentProgress) !== currentProgress) {
